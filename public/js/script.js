document.addEventListener('DOMContentLoaded', function () {
    ...
    const showFormSignUp = () => {
        ...
        document.querySelector('.login-text-content h2').innerHTML = 'Olá!<br>Seja bem-vindo(a)';
        document.querySelector('.login-text-content p').innerHTML = 'Crie sua conta para acessar a plataforma';
    };

    const showFormLogin = () => {
         document.querySelector('.login-form').style.display = 'flex';
         document.querySelector('.sign-up-form').style.display = 'none';
         document.querySelector('.login-text-content h2').innerHTML = 'Olá!<br>Seja bem-vindo(a) de volta';
         document.querySelector('.login-text-content p').innerHTML = 'Faça login para continuar acessando a página';
    ...
    loginForm.addEventListener('submit', function (event) {
        ...
        if(errors.length > 0) {
            ...
            fetch(`http://localhost:4000/users?email=${emailInput.value}&password=${passwordInput.value}`)

                .then((response) => response.json())
                .then((users) => {
                    if(users.length > 0) {
                        window.location = 'https://refatorando.com.br'
                    } else {
                        loginErrorMessage.style.display = 'none';
                        loginErrorMessage.innerText = 'E-mail ou Senha incorretos!';
                    }
                })
                .catch((error) => {
